...
namespace Game.Data
{
    public class GameMatchDataProvider : BaseDataProvider,IGameMatchProvider
    {
        ...
        public PagerSet GetRecordMatchInoutList(int pageIndex, int pageSize, string condition, string orderby)
        {
            PagerParameters parameters = new PagerParameters("vw_RecordMatchUserInout", orderby, condition, pageIndex, pageSize);
            return this.GetPagerSet2(parameters);
        }


        public DataTable GetMatchNoStat(int matchNo)
        {
            string str = "SELECT COUNT(DISTINCT UserID) AS UserCount,ISNULL(SUM(MatchFee),0) AS MatchFee,ISNULL(SUM(Revenue),0) AS Revenue,";
            str = str + "ISNULL(SUM(AwardGold),0) AS AwardGold,ISNULL(SUM(AwardMedal),0) AS AwardMedal,ISNULL(SUM(AwardExperience),0) AS AwardExperience" + string.Format(" FROM RecordMatchUserInout WHERE MatchNo={0}", matchNo);
            return Database.ExecuteDataset(str).Tables[0];
        }


        public DataSet GetMatchStat(string whereQuery)
        {
            string str = (string.Format("SELECT ServerID,SUM(MatchFee) AS MatchFee FROM RecordMatchUserInout(NOLOCK) {0} GROUP BY ServerID", whereQuery) + string.Format(";SELECT ServerID,SUM(Revenue) AS Revenue FROM RecordMatchUserInout(NOLOCK) {0} GROUP BY ServerID", whereQuery)) + string.Format(";SELECT ServerID,SUM(AwardGold) AS AwardGold FROM RecordMatchUserInout(NOLOCK) {0} GROUP BY ServerID", whereQuery) + string.Format(";SELECT ServerID,SUM(AwardMedal) AS AwardMedal FROM RecordMatchUserInout(NOLOCK) {0} GROUP BY ServerID", whereQuery);
            return Database.ExecuteDataset(CommandType.Text, str);
        }
