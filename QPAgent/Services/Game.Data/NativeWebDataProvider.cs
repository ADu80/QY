...
namespace Game.Data
{
    ...
    public class NativeWebDataProvider : BaseDataProvider, INativeWebDataProvider
    {
        ...
        private ITableProvider aidContent;
        private ITableProvider aideReward;
        #endregion
            ...
            : base( connString )
        {
            ...
            aidContent = GetTableProvider(Contents.Tablename);
            aideReward = GetTableProvider(Reward.Tablename);
        }
        #endregion
        ...
        /// <param name="orderby"></param>
        /// <returns></returns>
        public PagerSet GetContentist(int pageIndex, int pageSize, string condition, string orderby)
        {
            PagerParameters pagerPrams = new PagerParameters(Contents.Tablename, orderby, condition, pageIndex, pageSize);
        ...
        /// </summary>
        /// <param name="news"></param>
        public void InsertContent(Contents info)
        {
            DataRow dr = aidContent.NewRow();

            // dr[Contents._id] = info.Id;
            dr[Contents._TitleName] = info.TitleName;
            dr[Contents._DowUrl] = info.DowUrl;
            dr[Contents._Images] = info.Images;
            dr[Contents._TypeId] = info.TypeId;
            dr[Contents._PackageName] = info.PackageName;
            dr[Contents._Description] = info.Description;
            dr[Contents._addtime] = DateTime.Now;
            dr[Contents._iosAppName] = info.IosAppName;
            dr[Contents._iosIpaUrl] = info.IosIpaUrl;
            aidContent.Insert(dr);
        }
        ...
        /// </summary>
        /// <param name="news"></param>
        public void UpdateContent(Contents info)
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("UPDATE Content SET ")
                    .Append("TitleName=@TitleName ,")
                    .Append("DowUrl=@DowUrl,")
                    .Append("Images= @Images ,")
                    .Append("TypeId= @TypeId ,")
                    .Append("Description= @Description ,")
                    .Append("PackageName= @PackageName, ")
                    .Append("IosIpaUrl= @IosIpaUrl, ")
                    .Append("IosAppName= @IosAppName ")
                    .Append(" WHERE Id= @Id");

            var prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("TitleName", info.TitleName));

            prams.Add(Database.MakeInParam("DowUrl", info.DowUrl));

            prams.Add(Database.MakeInParam("Images", info.Images));

            prams.Add(Database.MakeInParam("TypeId", info.TypeId));

            prams.Add(Database.MakeInParam("Description", info.Description));

            prams.Add(Database.MakeInParam("PackageName", info.PackageName));

            prams.Add(Database.MakeInParam("IosIpaUrl", info.IosIpaUrl));

            prams.Add(Database.MakeInParam("IosAppName", info.IosAppName));

            prams.Add(Database.MakeInParam("id", info.Id));
        ...
        public PagerSet GetRewardList(int pageIndex, int pageSize, string condition, string orderby)
        {
            PagerParameters pagerPrams = new PagerParameters(Reward.Tablename, orderby, condition, pageIndex, pageSize);
        ...
        /// </summary>
        /// <param name="news"></param>
        public void UpdateReward(Reward Awardinfo)
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("UPDATE Reward SET ")
                    .Append("price=@price ,")
            ...
            var prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("price", Awardinfo.Price));
            prams.Add(Database.MakeInParam("addtime", Awardinfo.AddTime));

            prams.Add(Database.MakeInParam("Id", Awardinfo.Id));
        ...
        public Reward GetRewardInfo(int Id)
        {
            string sqlQuery = string.Format("(NOLOCK) WHERE Id= {0}", Id);
            Reward Awardinfo = aideReward.GetObject<Reward>(sqlQuery);
            if (Awardinfo != null)
                return Awardinfo;
            else
                return new Reward();
