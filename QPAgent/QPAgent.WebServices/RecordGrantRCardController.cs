...
{
    public class RecordGrantRCardController : ControllerBase
    {
        ...
        public override string New(HttpRequest req)
        {
            ...
            try
            {
                ...
                int GiftGold = Convert.ToInt32(req.Form["RCard"] ?? "0");
                ...
                if (IsAgent)
                {
                    msg = aideTreasureFacade.GrantRoomcard_Agent(ids, ids.Split(',').Length, GiftGold, userExt.AgentID, Reason, LoginIP);
                }
                else
                {
                    msg = aideTreasureFacade.GrantRoomcard(ids, GiftGold, userExt.UserID, Reason, LoginIP);
                }
                string json = "";
                if (msg.Success)
                {
                    json = JsonResultHelper.GetSuccessJson("操作成功");
                    LogHelper2.SaveSuccessLog("赠送[" + ids + "]房卡", userExt.UserID, (int)LogOperationEnum.EditAgent, GameRequest.GetUserIP(), module);
                }
                else
                {
                    if (msg.MessageID == -4)
                        msg.Content = "操作失败，可用房卡不足！";
                    json = msg.Content;
                    LogHelper2.SaveErrLog("赠送[" + ids + "]房卡", msg.Content, userExt.UserID, (int)LogOperationEnum.AddGamer, GameRequest.GetUserIP(), module);
                }
                return json;
            }
            catch (Exception ex)
