...
------表名：角色表------
------修改内容：增加字段 操作人，创建时间和修改时间
IF NOT EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='U' AND OBJECT_ID('SystemLog')=ID)
...
IF NOT EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='U' AND OBJECT_ID('SystemLogOperation')=ID)
...
--管理员
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='V' AND OBJECT_ID('vw_Base_Users')=ID)
...
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='V' AND OBJECT_ID('vw_SubAgentList')=ID)
...
IF(@AgentID<>'')
BEGIN
	SET @Where+=' AND a.AgentID = '+@AgentID
END
IF(@Nullity<>-1)
BEGIN
...
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='V' AND OBJECT_ID('vw_Log')=ID)
	DROP VIEW vw_Log
GO
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='V' AND OBJECT_ID('vw_SystemLog')=ID)
...
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='V' AND OBJECT_ID('vw_Agents_Tree')=ID)
...
BEGIN
	DECLARE @SQL VARCHAR(MAX)=''
	DECLARE @Where VARCHAR(MAX)=''
	DECLARE @tmpRecharge VARCHAR(50)
	DECLARE @tmpOnlineCount VARCHAR(50)
	DECLARE @tmpAllGamerCount VARCHAR(50)
	DECLARE @tmpAllGamerScore VARCHAR(50)
	DECLARE @tmpSubAgentCount VARCHAR(50)
	DECLARE @tempTable VARCHAR(50)
	SET @tmpRecharge = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	SET @tmpOnlineCount = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	SET @tmpAllGamerCount = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	SET @tmpAllGamerScore = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	SET @tmpSubAgentCount = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	SET @tempTable = Replace('##'+CONVERT(VARCHAR(50), NEWID()),'-','')
	...
	dbo.f_GetAgentWaste(a.UserID) AllWaste,
	dbo.f_GetAgentRecharge(a.UserID) Recharge,
	dbo.f_GetAgentOnLineCount(a.UserID) OnlineCount,
	dbo.f_GetAgentSubGamerCount(a.UserID) AllGamerCount,
	dbo.f_GetAgentSubGamerScore(a.UserID) AllGamerScore,
	dbo.f_GetSubAgentCount(a.UserID) SubAgentCount,
	Percentage,u.Nullity [Status],a.RegisterDate SpreaderDate,a.AgentID HigherAgentID,IsNull(a1.Accounts,''admin'') HigherAgentName,a.SpreaderID,ROW_NUMBER() OVER(ORDER BY a.UserID) RowNo
	INTO '+ @tempTable + ' 
	FROM QPAccountsDB.dbo.AccountsInfo a
	INNER JOIN [dbo].[Base_Users] u ON a.UserID=u.AgentID
...
BEGIN
	...
	SELECT *,dbo.f_GetGamerWasteByDate(UserID,GETDATE(),GETDATE()) GamerWasteSumToday,-dbo.f_GetGamerSpreaderSumByDate(UserID,GETDATE(),GETDATE()) GamerSpreaderSumToday FROM ' + @tempTable + '
	WHERE RowNo BETWEEN '+CONVERT(VARCHAR(10),(@pageIndex-1)*@pageSize)+' AND '+CONVERT(VARCHAR(10),@pageIndex*@pageSize)+'
	DROP TABLE '+@tempTable
...
IF NOT EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='U' AND OBJECT_ID('[AgentSpreaderOptions]')=ID)
...
BEGIN
	...
	SET @SQL='
	;WITH c AS(
		SELECT a.UserID,a.GameID,a.Accounts,a.NickName,a1.GameID SpreaderID,IsNull(a1.Accounts,''admin'') SpreaderName,IsNull(a1.NickName,''运营'') SpreaderNick
			,dbo.f_GetParentsSpreaderUserID(a.UserID) Relation,dbo.f_GetParentsSpreaderUserID2(a.UserID) Relation2,a2.GameID AgentID,IsNull(a2.Accounts,''admin'') AgentName,a.RegisterDate 
		FROM [QPAccountsDB].dbo.AccountsInfo a
		LEFT JOIN [QPAccountsDB].dbo.AccountsInfo a1 ON a.SpreaderID=a1.UserID
		LEFT JOIN [QPAccountsDB].dbo.AccountsInfo a2 ON a.AgentID=a2.UserID
		WHERE a.UserID='+CONVERT(VARCHAR(30),@UserID)+'
		UNION ALL 
		SELECT b.UserID,b.GameID,b.Accounts,b.NickName,a1.GameID SpreaderID,IsNull(a1.Accounts,'''') SpreaderName,IsNull(a1.NickName,''运营'') SpreaderNick
			,dbo.f_GetParentsSpreaderUserID(b.UserID) Relation,dbo.f_GetParentsSpreaderUserID2(b.UserID) Relation2,a2.GameID AgentID,IsNull(a2.Accounts,'''') AgentName,b.RegisterDate 
		FROM [QPAccountsDB].dbo.AccountsInfo b
		INNER JOIN [QPAccountsDB].dbo.AccountsInfo a1 ON b.SpreaderID=a1.UserID
		INNER JOIN [QPAccountsDB].dbo.AccountsInfo a2 ON b.AgentID=a2.UserID
		INNER JOIN c ON b.SpreaderID=c.UserID
	)	
	SELECT *,Row_Number() OVER(Order BY UserID DESC ) RowNo INTO '+@tempTable+' FROM c
	WHERE UserID<>'+CONVERT(VARCHAR(30),@UserID)+'
...
BEGIN
	UPDATE u
	SET u.Nullity=@Nullity,u.UserName=@UserName,u.canHasSubAgent=@canHasSubAgent,u.AgentLevelLimit=@AgentLevelLimit,u.Percentage=@Percentage,u.RoleID=@RoleID,u.GradeID=@GradeID
	FROM Base_Users u
	WHERE u.UserID=@UserID
END
...
IF NOT EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE [TYPE]='U' AND OBJECT_ID('SensitiveWordSet')=ID)
