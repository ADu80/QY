...
-- 帐号登录
IF EXISTS(SELECT 1 FROM dbo.SYSOBJECTS WHERE OBJECT_ID('NET_PW_EfficacyAccounts_ByMobile')=id AND xtype='P')
	DROP PROC NET_PW_EfficacyAccounts_ByMobile
GO
CREATE PROCEDURE dbo.NET_PW_EfficacyAccounts_ByMobile
	@strMobile NVARCHAR(31),					-- 用户帐号
...
BEGIN
	...
	FROM AccountsInfo(NOLOCK) WHERE RegisterMobile=@strMobile

	-- 查询用户
	IF @UserID IS NULL
	BEGIN
		SET @strErrorDescribe=N'您的帐号不存在或者密码输入有误，请查证后再试！'
		RETURN 2
	END	
