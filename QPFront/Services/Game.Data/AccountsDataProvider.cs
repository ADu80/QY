...
namespace Game.Data
{
    ...
    public class AccountsDataProvider : BaseDataProvider, IAccountsDataProvider
    {
        ...
        /// <param name="names">用户信息</param>
        /// <returns></returns>
        public Message LoginByMobile(UserInfo user)
        {
            List<DbParameter> prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("strMobile", user.RegisterMobile));
            prams.Add(Database.MakeInParam("strPassword", user.LogonPass));
            prams.Add(Database.MakeInParam("strClientIP", user.LastLogonIP));
            prams.Add(Database.MakeOutParam("strErrorDescribe", typeof(string), 127));

            return MessageHelper.GetMessageForObject<UserInfo>(Database, "NET_PW_EfficacyAccounts_ByMobile", prams);
        }

        public string GetAccountsByMobile(string mobile)
        {
            List<DbParameter> parms = new List<DbParameter>();
            parms.Add(Database.MakeInParam("RegisterMobile", mobile));
            string sql = "SELECT Accounts from AccountsInfo WHERE RegisterMobile=@RegisterMobile";
            object o = Database.ExecuteScalar(CommandType.Text, sql, parms.ToArray());
            return o.ToString();
        }
        ...
        /// <returns></returns>
        public UserInfo GetAccountInfoByAccount(string account)
        {
            List<DbParameter> prams = new List<DbParameter>();
            prams.Add(Database.MakeInParam("uAccount", account));
            prams.Add(Database.MakeOutParam("strErrorDescribe", typeof(string), 127));

            return Database.RunProcObject<UserInfo>("NET_PW_GetUserBaseInfoByAccount", prams);
        }


        public DataSet GetAccountInfoByGameID(string sqlQuery)
        {
        ...
        public DataTable GetAccountScoreInfo(Dictionary<string, object> condition)
        {
            List<DbParameter> paramlist = new List<DbParameter>();
            string whereAppend = "";
            foreach (var item in condition)
            {
                whereAppend += " AND " + item.Key + "=@" + item.Key;
                paramlist.Add(Database.MakeInParam(item.Key, item.Value));
            }
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("SELECT a.UserID,a.GameID,a.Accounts,a.NickName,a.MemberOrder,s.Score,s.Diamond,s.RCard,0 Point,a.Gender");
            sb.AppendLine("FROM QPAccountsDB.dbo.AccountsInfo a");
            sb.AppendLine("INNER JOIN [QPTreasureDB].[dbo].[GameScoreInfo] s ON a.UserID=s.UserID");
            sb.AppendLine("WHERE 0=0 " + whereAppend);
            DataTable dt = Database.ExecuteDataset(CommandType.Text, sb.ToString(), paramlist.ToArray()).Tables[0];
            return dt;
        }
        public DataTable GetAccountScoreInfoByUserID(int id)
        {
            Dictionary<string, object> cond = new Dictionary<string, object>();
            cond.Add("UserID", id);
            return GetAccountScoreInfo(cond);
        }
        public DataTable GetAccountScoreInfoByGameID(int id)
        {
            Dictionary<string, object> cond = new Dictionary<string, object>();
            cond.Add("GameID", id);
            return GetAccountScoreInfo(cond);
        }
        public DataTable GetAccountScoreInfoByAccount(string account)
        {
            Dictionary<string, object> cond = new Dictionary<string, object>();
            cond.Add("Accounts", account);
            return GetAccountScoreInfo(cond);
        }
        public DataTable GetAccountScoreInfoByNick(string nick)
        {
            Dictionary<string, object> cond = new Dictionary<string, object>();
            cond.Add("NickName", nick);
            return GetAccountScoreInfo(cond);
        }
    }
}
