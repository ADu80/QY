    ...
    public class NativeWebDataProvider : BaseDataProvider, INativeWebDataProvider
    {
        ...
        public GameRulesInfo GetGameHelp(int kindID)
        {
            ...
            sqlQuery.Append("SELECT KindID, KindName, ImgRuleUrl, HelpIntro, HelpRule, HelpGrade, JoinIntro, Nullity, CollectDate, ModifyDate ")
                    .Append("FROM GameRulesInfo ")
                    .AppendFormat("WHERE KindID={0} AND Nullity=0 ", kindID)
                    .Append(" ORDER By CollectDate DESC");
        ...
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("SELECT NewsID,Subject,IsHot,ClassID,CONVERT(varchar(100), IssueDate, 21) IssueDate FROM [QPNativeWebDB].[dbo].[News] ORDER By IssueDate DESC ");
        ...
        /// <param name="IssueDate"></parassm>
        /// <returns></returns>
        public DataTable GetNewTypeList(int NewsID, string Subject, int ClassID, string IssueDate)
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("select NewsID,Subject,ClassID,CONVERT(varchar(100), GETDATE(), 21) from [QPNativeWebDB].[dbo].[News] ORDER By IssueDate DESC");
            return Database.ExecuteDataset(sqlQuery.ToString()).Tables[0];
        }
        ...
        public DataTable GetNewDetailList(int id)
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append(@"
select  NewsID, Subject,n.UserID, CONVERT(varchar(100), IssueDate, 20) IssueDate, Body, Username 
FROM [QPNativeWebDB].[dbo].[News]  n
LEFT JOIN[QPPlatformManagerDB].[dbo].[Base_Users] u
ON n.UserID = u.UserID WHERE n.NewsID=" + id + @"
ORDER By IssueDate DESC");
        ...
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("select FeedbackID,Accounts,FeedbackContent from GameFeedbackInfo");
            return Database.ExecuteDataset(sqlQuery.ToString()).Tables[0];
        }
        /// <summary>
        /// 游戏建议
        ...
        public Message InsertGameFeedback(GameFeedbackInfo info)
        {
            ...
            parms.Add(Database.MakeInParam("strTitle", info.FeedbackTitle));
            parms.Add(Database.MakeInParam("strContent", info.FeedbackContent));
            parms.Add(Database.MakeInParam("strphone", info.Phone));
            parms.Add(Database.MakeInParam("strClientIP", info.ClientIP));
            parms.Add(Database.MakeOutParam("strErrorDescribe", typeof(string), 127));
            parms.Add(Database.MakeInParam("strEMail", info.EMail));
            parms.Add(Database.MakeInParam("strQQ", info.QQ));
            return MessageHelper.GetMessage(Database, "NET_PW_InsertGameFeedback", parms);
        }
        ...
        {
            List<DbParameter> parms = new List<DbParameter>();
            parms.Add(Database.MakeInParam("ftype", ftype));
            parms.Add(Database.MakeInParam("strAccounts", info.Accounts));
            parms.Add(Database.MakeInParam("strTitle", info.FeedbackTitle));
            parms.Add(Database.MakeInParam("strContent", info.FeedbackContent));
            parms.Add(Database.MakeInParam("strphone", info.Phone));
        ...
        {
            StringBuilder sqlQuery = new StringBuilder();
            sqlQuery.Append("select * from GameIssueInfo ORDER By CollectDate DESC");
            return Database.ExecuteDataset(sqlQuery.ToString()).Tables[0];
        }
        #endregion
    }
